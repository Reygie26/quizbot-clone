...
async def send_quiz_to_group(chat_id, quiz_id, context):
    ...
    keyboard = InlineKeyboardMarkup([
        [
            InlineKeyboardButton(
                "▶️ Start Quiz",
                url=f"https://t.me/EucresiaBot?start=PLAY_{quiz_id}"
            )
        ]
    ])
...
async def update_group_leaderboard(quiz_id, context):
    ...
    buttons.append([
        InlineKeyboardButton("▶️ Start this Quiz", url=f"https://t.me/EucresiaBot?start=PLAY_{quiz_id}")
    ])
...
app.add_handler(CallbackQueryHandler(send_quiz_to_group, pattern="^POST_QUIZ$"))
